...
\uncover<3->{
We call the objects in $Z$ the \alert{elements} of $Z$.
}
...
\uncover<5->{
with ``cow is an element of $Z$''.
}

\uncover<6->{
Frequently encountered sets are
\[
\begin{split}
...
\uncover<13->{\mathbb{Q}} \uncover<14->{= \{p/q : p,q\in\mathbb{Z},\ q\neq 0\}}&
  \uncover<15->{\qquad (\text{``rational numbers''})\\}
\uncover<16->{\mathbb{R}} \uncover<17->{= \{\hbox{decimal numbers}\}}&
  \uncover<18->{\quad\qquad (\text{``real numbers''})}
\end{split}
\]
}
\end{frame}
